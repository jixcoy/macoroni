(function() {
    var _onload = function() {
      ...
      var canvasframe=function() {
        ...
        for(var j=0;j<6.28;j+=.07) { // j <=> theta
          var ct=Math.cos(j),st=Math.sin(j); // cosine theta, sine theta
          for(i=0;i<3.64;i+=0.01) {   // i <=> phi
            var sp=Math.sin(i),cp=Math.cos(i); // cosine phi, sine phi
            var ox = R2 + R1*ct, // object x, y = (R2,0,0) + (R1 cos theta, R1 sin theta, 0)
                oy = R1*st;
    
            var x = 4*(ox*(cB*cp + sA*sB*sp) - oy*cA*sB); // final 3D x coordinate
            var y = 4*(ox*(sB*cp - sA*cB*sp) + oy*cA*cB); // final 3D y
            var ooz = 1/(K2 + cA*ox*sp + sA*oy); // one over z
            var xp=(window.innerWidth/2+K1*ooz*x); // x' = screen space coordinate, translated and scaled to fit our 320x240 canvas element
            var yp=(window.innerHeight/2-K1*ooz*y); // y' (it's negative here because in our output, positive y goes down but in our 3D space, positive y goes up)
            // luminance, scaled back to 0 to 1
            var L=0.7*(cp*ct*sB - cA*ct*sp - sA*st + cB*(cA*st - ct*sA*sp));
            if(L > 0) {
              ctx.fillStyle = 'rgba(255,200 ,0,'+L+')';
              ctx.fillRect(xp, yp, 15, 15);
            }
          }
        }
      }
